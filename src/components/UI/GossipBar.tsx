...
export const GossipBar = () => {
  const gossip = useSpiritGossipStore((s) => s.gossip);

  if (!gossip) return null;

  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-black/70 text-white px-5 py-3 rounded-xl shadow-lg border border-white/20 backdrop-blur-md flex items-center gap-3 z-40 max-w-[90vw]">
      <img
      ...
      <div className="text-sm leading-tight">
        ...
        <div className="text-white">{gossip.text}</div>
      </div>
    </div>
  );
};
