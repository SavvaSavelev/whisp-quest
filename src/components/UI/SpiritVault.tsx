...
export function SpiritVault({ isOpen, onClose, onSelectSpiritForChat }: SpiritVaultProps) {
  ...
  const filteredSpirits = useMemo(() => {
    ...
    if (filter === 'favorites') {
      ...
      filtered = filtered.filter(spirit => 
        spirit.mood === filter || spirit.rarity === filter
      );
    }
  ...
  const getRarityGlow = (rarity: string) => {
    switch (rarity) {
      case 'легендарный': return 'shadow-[0_0_30px_#ffd700,0_0_60px_#ffd700]';
          ...
          <div className="relative z-10 p-3 bg-black/20 backdrop-blur-sm border-b border-white/10">
            <div className="flex flex-wrap gap-3 items-center justify-between text-sm">
              ...
              <div className="flex gap-2 items-center">
                ...
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as 'recent' | 'rarity' | 'mood')}
                  className="bg-slate-800/50 text-white px-3 py-1 rounded-lg border border-slate-600/50 text-sm backdrop-blur-sm"
                >
              ...
              <div className="flex gap-2">
                {(['galaxy', 'constellations', 'grid', 'list'] as const).map((mode) => (
                  <button
                    key={mode}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      ...
                      const randomSpirit = filteredSpirits[Math.floor(Math.random() * filteredSpirits.length)];
                      if (randomSpirit) {
                  ...
                  <button
                    onClick={() => {
                      if (confirm('Удалить всех духов? Это действие нельзя отменить!')) {
                        clearArchive();
                      }
                    }}
          ...
          <div className="flex-1 relative z-10 p-6 overflow-y-auto">
            {filteredSpirits.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <h3 className="text-xl text-slate-300 mb-2">Пусто как в космосе</h3>
                  ...
                  <div className="relative">
                    ...
                    <div className="relative w-96 h-96 mx-auto">
                      ...
                      {showConnections && filteredSpirits.length > 1 && (
                        <svg className="absolute inset-0 w-full h-full pointer-events-none">
                          {filteredSpirits.slice(0, 10).map((spirit, i) => 
                            filteredSpirits.slice(i + 1, 10).map((otherSpirit) => {
                              if (spirit.mood === otherSpirit.mood || spirit.rarity === otherSpirit.rarity) {
                                return (
                                  <line
                              ...
                              >
                                <div className={`relative group ${isSelected ? 'animate-pulse' : ''}`}>
                                  <img
                                    ...
                                    alt={spirit.essence}
                                    className={`w-16 h-16 rounded-full border-2 border-yellow-400 ${getRarityGlow(spirit.rarity)} ${
                                      isSelected ? 'ring-4 ring-yellow-300/50' : ''
                                    }`}
                                  />
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      const newFavorites = new Set(favoriteSpirits);
                                      if (isFavorite) {
                                    ...
                                    className={`absolute -top-1 -right-1 w-6 h-6 rounded-full ${
                                      isFavorite ? 'bg-red-500 text-white' : 'bg-black/50 text-gray-400'
                      ...
                      <div className="absolute inset-[-100px] rounded-full border border-purple-400/20">
                        {filteredSpirits
                          .filter(spirit => ['эпический', 'редкий'].includes(spirit.rarity))
                          .map((spirit, index, arr) => {
                            const angle = (index / arr.length) * 360;
                            const radius = 180;
                            const x = Math.cos(angle * Math.PI / 180) * radius;
                                ...
                                <div className="relative group">
                                  <img
                                    ...
                                    className={`w-12 h-12 rounded-full border-2 ${
                                      spirit.rarity === 'эпический' ? 'border-purple-400' : 'border-blue-400'
                              ...
                              >
                                <div className="relative group">
                                  <img
                                    src={getMoodTexture(spirit.mood)}
                                    alt={spirit.essence}
                                    className={`w-10 h-10 rounded-full border border-green-400 ${getRarityGlow(spirit.rarity)}`}
                                  />
                                  <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/80 text-white px-2 py-1 rounded text-xs whitespace-nowrap">
                                    {spirit.essence}
                                  </div>
                ...
                {viewMode === 'constellations' && (
                  <div className="space-y-8">
                    <div className="text-center mb-8">
                      ...
                      <p className="text-slate-400">Духи сгруппированы по настроениям в мистические созвездия</p>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 justify-items-center">
                      {['радостный', 'печальный', 'злой', 'спокойный', 'вдохновленный'].map((mood) => (
                    ...
                    {(() => {
                      const knownMoods = ['радостный', 'печальный', 'злой', 'спокойный', 'вдохновленный'];
                      const unknownMoods = [...new Set(filteredSpirits.map(s => s.mood))].filter(mood => !knownMoods.includes(mood));
                      
                      if (unknownMoods.length > 0) {
                        return (
                          <div className="mt-12">
                            <h4 className="text-lg font-bold text-center text-slate-300 mb-6">🌟 Особые созвездия 🌟</h4>
                      ...
                      >
                        <div className="text-center">
                          <img
                            src={getMoodTexture(spirit.mood)}
                            alt={spirit.essence}
                            className={`w-20 h-20 mx-auto mb-3 rounded-full border-2 ${getRarityGlow(spirit.rarity)}`}
                          />
                          
                          <h3 className="text-white font-bold text-sm mb-1 truncate">{spirit.essence}</h3>
                          
                          <div className={`inline-block px-2 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${getRarityColor(spirit.rarity)}`}>
                            ⭐ {spirit.rarity}
                          </div>
                          
                          <p className="text-slate-400 text-xs mt-2">🎭 {spirit.mood}</p>
                    ...
                    {filteredSpirits.map((spirit) => (
                      ...
                      >
                        <img
                          src={getMoodTexture(spirit.mood)}
                          alt={spirit.essence}
                          className={`w-16 h-16 rounded-full border-2 ${getRarityGlow(spirit.rarity)}`}
                        />
                        
                        <div className="flex-1">
                          <h3 className="text-white font-bold text-lg">{spirit.essence}</h3>
                          <div className="flex items-center gap-4 mt-1">
                            <span className={`px-2 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${getRarityColor(spirit.rarity)}`}>
                              ⭐ {spirit.rarity}
                            </span>
                            <span className="text-slate-400 text-sm">🎭 {spirit.mood}</span>
                            {spirit.birthDate && (
                              <span className="text-slate-500 text-sm">
                                🕯️ {format(new Date(spirit.birthDate), 'd.MM.yy HH:mm')}
                              </span>
                            )}
                          </div>
                          {spirit.originText && (
